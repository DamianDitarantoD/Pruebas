...
if(isset($_POST["ID"]) && !empty($_POST["ID"])){
    ...
    if(empty($name_err) && empty($Dep_err) && empty($address_err)){
        ...
        $sql = "UPDATE Marcas_Modelos SET Marca=?, Modelo=? WHERE ID=?";
    ...
    if(isset($_GET["ID"]) && !empty(trim($_GET["ID"]))){
        ...
        $sql = "SELECT * FROM Marcas_Modelos WHERE ID = ?";
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post">
                            ...
                            <label>Modelo</label>
                            <input type="text" name="Modelo" class="form-control" value="<?php echo $Dep; ?>">

...
if(isset($_POST["ID"]) && !empty($_POST["ID"])){
    ...
    if(empty($name_err) && empty($Dep_err) && empty($address_err)){
        ...
        $sql = "UPDATE Funcion SET  Nombre=? WHERE ID=?";
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                 ...
                 header("location: http://192.168.1.80/Funcion/Funcion.php");
                 exit();
            } else{
    ...
    if(isset($_GET["ID"]) && !empty(trim($_GET["ID"]))){
        ...
        $sql = "SELECT * FROM Funcion WHERE ID = ?";
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post">
                        ...
                        <a href="http://192.168.1.80/Funcion/Funcion.php" class="btn btn-default">Cancelar</a>
                    </form>
                </div>
            </div>        
        </div>

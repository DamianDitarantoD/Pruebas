...
{
	header("location: http://192.168.1.80/admin/admin_portada.php");	
}
if(isset($_SESSION["personal_login"]))	//Condicion personal
{
	header("location: http://192.168.1.80/personal/personal_portada.php"); 
}
if(isset($_SESSION["usuarios_login"]))	//Condicion Usuarios
{
	header("location: http://192.168.1.80/usuarios/usuarios_portada.php");
}
...
if(isset($_REQUEST['btn_login']))	
{
	...
	else if($email AND $password /*AND $role*/)
	{
		try
		{
			$select_stmt=$db->prepare("SELECT * FROM mainlogin
										WHERE
					...
					if($email==$dbemail and $password==$dbpassword /*and $role==$dbrole*/) 
					{
						switch($dbrole)		
						{
							case "admin": 
								...
								header("refresh:3;http://192.168.1.80/admin/admin_portada.php");	
								break;
								
							case "personal":
								$_SESSION["personal_login"]=$email;				
								$loginMsg="Personal: Inicio sesión con éxito";		
								header("refresh:3;http://192.168.1.80/personal/personal_portada.php");	
								break;
								
							case "usuarios":
								$_SESSION["usuarios_login"]=$email;				
								$loginMsg="Usuario: Inicio sesión con éxito";	
								header("refresh:3;http://192.168.1.80/usuarios/usuarios_portada.php");		
								break;
...
<centerr><h2>Iniciar sesión</h2></centerr>

...
if(isset($_POST["id"]) && !empty($_POST["id"])){
    ...
    if($stmt = mysqli_prepare($link, $sql)){
        ...
        if(mysqli_stmt_execute($stmt)){
            ...
            header("location: http://192.168.1.80/Proyectos/Proyectos.php");
            exit();
        } else{
                    ...
                    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                        <div class="alert alert-danger fade in">
                            ...
                            <b>Está seguro que quieres eliminar a:</></br></b>
                                <?php 
                                 $loc = $_GET["id"];
                                  $sql = "SELECT * FROM Proyectos WHERE ID = $loc";
                                  if($result = mysqli_query($link, $sql)){
                                    ...
                                    <a href="http://192.168.1.80/Proyectos/Proyectos.php" class="btn btn-default">No</a>
                                </h4>
                            </div>
                        </form>
                    </div>

...
if(isset($_POST["ID"]) && !empty($_POST["ID"])){
   ...
   // $input_address = trim($_POST["Apellidos"]);
   ...
   // }
    ...
    // Validate salary
   // $input_salary = trim($_POST["Movil_Personal"]);
   ...
   // } elseif(!ctype_digit($input_salary)){
    ...
    if(empty($name_err)){
        ...
        //$sql = "UPDATE Departamentos SET Nombre=?, Apellidos=?, Movil_Personal=? WHERE ID=?";
        ...
        if($stmt = mysqli_prepare($link, $sql)){
           ...
           // $param_address = $address;
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                echo "Something went wrong. Please try again later 9999999.";
            }
        }
                ...
                <div class="col-md-12">
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post">
                       ...
                       <!-- <div class="form-group <?php echo (!empty($address_err)) ? 'has-error' : ''; ?>">
                            ...
                            <textarea name="Apellidos" class="form-control"><?php echo $address; ?></textarea>
                            <span class="help-block"><?php echo $address_err;?></span>
                        </div>
                        <div class="form-group <?php echo (!empty($salary_err)) ? 'has-error' : ''; ?>">
                            <label>Sueldo</label>
                            <input type="text" name="Movil_Personal" class="form-control" value="<?php echo $salary; ?>">
                            <span class="help-block"><?php echo $salary_err;?></span>
                        </div>-->
                        <input type="submit" name="ID" value="<?php echo $id; ?>"/>
                        <input type="submit" class="btn btn-primary" value="Enviar">
                        <a href="http://192.168.1.80/index.php" class="btn btn-default">Cancelar</a>
                    </form>
                </div>
            </div>        
        </div>

...
if(isset($_POST["ID"]) && !empty($_POST["ID"])){
    ...
    // ValIDate address address
    $input_address = trim($_POST["Operador"]);
    if(empty($input_address)){
        $address_err = "Por favor ingrese una dirección.";     
    } else{
        $Opera = $input_address;
    ...
    if(empty($name_err) && empty($Dep_err) && empty($address_err)){
        ...
        $sql = "UPDATE Costes_Internet SET  Nombre=?, Importe=?, Departamentos_ID=?, Operador=? WHERE ID=?";

        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssis", $param_name, $param_Dep, $input_address, $param_ID, $param_Opera);
                        // Set parameters
                        $param_name = $name;
                        $param_Dep = $Dep;
            ...
            if(mysqli_stmt_execute($stmt)){
                 ...
                 header("location: http://192.168.1.80/Internet/Internet.php");
                 exit();
            } else{
    ...
    if(isset($_GET["ID"]) && !empty(trim($_GET["ID"]))){
        ...
        $sql = "SELECT * FROM Costes_Internet WHERE ID = ?";
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post">
                            ...
                            <input type="text" name="Operador" class="form-control" value="<?php echo $Opera; ?>">
                          
                        <input type="hidden" name="ID" value="<?php echo $ID; ?>"/>
                        <input type="submit" class="btn btn-primary" value="Enviar">
                        <a href="http://192.168.1.80/Internet/Internet.php" class="btn btn-default">Cancelar</a>
                    </form>
                </div>
            </div>        
        </div>
